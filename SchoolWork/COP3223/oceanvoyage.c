...
void setup(char crewnames[NUMCREW][STRLENGTH], int crewstatus[NUMCREW], int supplies[NUMSUPPLIES], int *captaintype, int *funds, int *distanceperday) {
//Opening statement. Ask for captain type.
  printf("You may now take your ship and crew from Port Marin, Spain to the hidden island in the Caribbean on the old pirate's ship.\n\n")
  printf(" How will you travel??\n");
  printf("1.) As a merchant.\n");
  printf("2.) As a privateer.\n");
  printf("3.) As a pirate.\n");
  printf("Captain Choices:\n");

  printf("As a merchant, you begin your trip with 1000 gold pieces. You will be sailing your Carrack, with an average speed of 80 miles per day.\n");
  printf("As a privateer, you begin your trip with 900 gold pieces. You will be sailing your Galleon, with an average speed of 90 miles per day.\n");
  printf("As a pirate, you begin your trip with 800 gold pieces. You will be sailing your Clipper, with an average speed of 100 miles per day.\n");

// While loop getting captain type if answer is not 1, 2, or 3, loop and get another answer.
do
  scanf("%d", &captaintype);
  if(captaintype == 1)
  {
  ...
  {
    printf("You did not enter a correct choice.\n Enter a choice between 1 and 3.\n")
  }
  while(captaintype <= 0 && captaintype > 3);

//Get Captain name. Set captains status to 2;
  printf("What is your name, Captain?"\n);
  scanf("%s", crewnames[0]);
  crewstatus[0] = 2;

//Get names of crewmembers. Set crewmembers' status to 2.
  printf("Who are the other members of your crew?\n");
  int i;
  for(i = 1; i < NUMCREW; i++)
  {
    printf("Enter name for crew member %d.\n", i);
    scanf("%s", crewnames[i]);
    crewstatut[i] = 2;
  }
...
int num, supplyChoice, total;
  ...
  do {
    printf("Enter Choice:\n");
    scanf("%d", &supplyChoice);

  //Calculate each choice. Multiply the number of supplies by how much each of those supplies costs.
  //If the total is more than the funds left printf out statement.
  //Otherwise place the amount into the proper position of supplies[number]
  //And subtract total from funds
  switch(supplyChoice)
  {
    case 1:
      printf("%s\n", supplytypes[supplyChoice]);
      printf("How many pounds of %s do you want to buy?\n", supplytypes[supplyChoice] );
      scanf("%d", &num);
      total = num * supplycosts[supplyChoice];
      if(total > funds)
      {
        printf("Sorry, You cannot afford that much %s\n", supplytypes[supplyChoice]);
      }
    ...
    case 2:
      printf("%s\n", supplytypes[supplyChoice]);
      printf("How many sets of %s do you want to buy?\n", supplytypes[supplyChoice]);
      scanf("%d", &num);
      total = num * supplycosts[supplyChoice];
      if(total > funds)
      {
        printf("Sorry, You cannot afford that many sets of %s\n", supplytypes[supplyChoice]);
      }
    ...
    case 3:
      printf("%s\n", supplytypes[supplyChoice]);
      printf("How many extra %s do you want to buy?\n", supplytypes[supplyChoice]);
      scanf("%d", &num);
      total = num * supplycosts[supplyChoice];
      if(total > funds)
      {
        printf("Sorry, You cannot afford that many %s\n", supplytypes[supplyChoice]);
      }
...
void dailyreport(char crewnames[NUMCREW][STRLENGTH], int crewstatus[NUMCREW], int supplies[NUMSUPPLIES], int funds, int traveled) {
  ...
  printf("Daily Status Report:\n");
  printf("You have traveled %d miles.\n", traveled);
  printstatus(char crewnames[NUMCREW][STRLENGTH], int crewstatus[NUMCREW]);
  printf("You have %d gold pieces.\n", funds);
  printf("You have %d pound of food.\n", supplies[0]);
...
//  nothing happens.
void rest(int supplies[NUMSUPPLIES], char crewnames[NUMCREW][STRLENGTH], int crewstatus[NUMCREW], int *days) {
int numDays, num, member;
  printf("How many days would you like to rest?\n");
  scanf("%d", &numDays);
  days -= numDays;
  supplies[0] -= (numDays * 2);

  num = rand() % 2;
  member = rand() % 6;

  if(num == 1 && crewstatus[member] == 2)
  {
